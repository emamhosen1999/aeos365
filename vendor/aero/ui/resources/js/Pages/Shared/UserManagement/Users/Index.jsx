...
/**
 * Shared, context-parameterized Users index. Renders identically for every
 * calling context (tenant admin, platform admin, …) — only the route-name
 * prefix, HRMAC gate namespace, and scope-gated capabilities differ, all
 * supplied by the controller as page props.
 *
 * Prop contract:
 * - routePrefix    the route-name prefix every action builds on (e.g. an
 *                   `<x>.users` group) — see fallback below for the shape.
 * - hrmacNamespace the HRMAC gate namespace every `useHRMAC()` call builds on.
 * - scope          'tenant' | 'platform'
 * - capabilities   { impersonation: bool, invitations: bool }
 * - dashboardRoute breadcrumb target (falls back per scope)
 */
export default function UsersIndex({
  users, roles, invitations, filters, stats,
  routePrefix, hrmacNamespace, scope, capabilities, dashboardRoute,
}) {
  ...
  const canView              = useHRMAC(`${hrmacNamespace}.users.view`);
  ...
  const canBulkDelete        = useHRMAC(`${hrmacNamespace}.users.bulk_delete`);
  ...
  // table) and pending invitations (a different view, tenant-only). `view`
  // switches datasets.
  ...
  // Invitations are a tenant-only capability — gated below.
  const [inviteOpen, setInviteOpen] = useState(false);
  const inviteForm = useForm({ email: '', roles: [] });

  const submitInvite = e => {
    e.preventDefault();
    inviteForm.post(route(`${routePrefix}.invitations.store`), {
      preserveScroll: true,
  ...
  const applyFilters = () => {
    router.get(route(`${routePrefix}.index`), { search, status, role }, {
  ...
  const toggleUserStatus = (id, activate) => {
    router.put(route(`${routePrefix}.toggle-status`, id), { active: activate }, {
      preserveState: true,
  ...
  const handleBulkToggle = activate => {
    ...
    router.post(route(`${routePrefix}.bulk.toggle-status`), { user_ids: selectedIds, active: activate }, {
      preserveState: true,
  ...
  const handleBulkDelete = () => {
    ...
    router.post(route(`${routePrefix}.bulk.delete`), { user_ids: selectedIds }, {
  ...
  const impersonateUser = id => {
    router.post(route(`${routePrefix}.impersonate`, id), {}, {
  ...
  const columns = [
    ...
    { key: 'created_at', label: 'Joined', width: '12%', render: row => new Date(row.created_at).toLocaleDateString() },
    {
      ...
      render: row => {
        ...
        const menuItems = [
          canView && { label: 'View profile', onClick: () => router.visit(route(`${routePrefix}.show`, row.id)) },
        ...
        return (
          <HStack gap={1} justify="end" align="center">
            {canEdit && (
              <Button intent="soft" size="sm" onClick={() => router.visit(route(`${routePrefix}.edit`, row.id))}>
                Edit
              </Button>
            )}
            {menuItems.length > 0 && (
  ...
  const showingInvites = capabilities.invitations && view === 'invitations';
  ...
  if (capabilities.invitations) {
    tabs.push({ value: 'invitations', label: 'Pending invitations', count: stats?.pending });
  }
  ...
  return (
    ...
    <IndexPageLayout
      ...
      tabs={
        <Tabs
          ...
          onChange={v => {
            ...
            setStatus(newStatus);
            router.get(route(`${routePrefix}.index`), { search, status: newStatus, role }, {
              preserveState: true, preserveScroll: true, only: ['users', 'filters'],
            });
          }}
        />
      ...
      actions={
        canCreate && (
          <HStack gap={2}>
            <Button intent="ghost" onClick={() => router.visit(route(`${routePrefix}.create`))}>
              Create user
            </Button>
          ...
          {!tableLoading && userRows.length === 0 ? (
            hasActiveFilter ? (
              ...
              <EmptyState
                icon="users"
                title="No users yet"
                description={capabilities.invitations
                  ? 'Invite your first teammate by email, or create an account directly.'
                  : 'Create your first user account to get started.'}
                action={capabilities.invitations
                  ? (canCreate && <Button intent="primary" onClick={() => setInviteOpen(true)}>Invite user</Button>)
                  : (canCreate && <Button intent="primary" onClick={() => router.visit(route(`${routePrefix}.create`))}>Create user</Button>)}
              />
            )
          ) : (
      ...
      pagination={
        !showingInvites && users?.last_page > 1 && (
          <Pagination
            ...
            onChange={page => router.get(route(`${routePrefix}.index`), { page, search, status, role }, {
...
UsersIndex.layout = page => (
  <App title="Users" railTitle="User management" rail={
    <UsersRail
