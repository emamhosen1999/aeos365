...
export default function StepVerifyEmail({ email, companyName }) {
    ...
    const verify = async () => {
        ...
        try {
            await axios.post(route('platform.register.verify-email.verify'), { code });
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="display-section text-3xl">Verify Company Email</h1>
                <p className="mt-2 text-[var(--pub-text-muted)]">
                    We will send a 6-digit code to <span className="text-white">{email}</span> for {companyName}.
                </p>
            </div>
