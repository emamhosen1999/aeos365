 ...
 * Shows overdue corrective and preventive actions.
 */
export default function OverdueCapasWidget({ 
    total = 0,
    due_this_week = 0,
    assigned_to_me = 0,
    loading = false,
    show_more_url = '/quality/capa',
    title = 'Overdue CAPAs'
}) {
    if (loading) {
        return (
            ...
            <CardBody className="p-4">
                {total === 0 ? (
                    <div className="flex flex-col items-center justify-center py-6 text-center">
                        <ExclamationCircleIcon className="w-12 h-12 text-default-300 mb-2" />
                        <p className="text-default-500">No overdue CAPAs</p>
                    </div>
                ) : (
                    <div className="space-y-3">
                        <div className="grid grid-cols-3 gap-3">
                            ...
                            <div className="text-center p-3 rounded-lg bg-primary-50 dark:bg-primary-900/20">
                                ...
                                <p className="text-xs text-default-600">My CAPAs</p>
                            </div>
                        </div>
