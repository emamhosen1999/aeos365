 ...
 */
class WelcomeEmployeeNotification extends BaseHrmNotification
{
    protected string $eventType = 'employee_welcome';
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $employeeName = $notifiable->name ?? 'Team Member';
        $employeeCode = $this->employee->employee_code ?? 'N/A';
        ...
        $startDate = $this->employee->date_of_joining?->format('F d, Y') ?? 'soon';
        $daysRemaining = $this->onboarding->expected_completion_date?->diffInDays(now()) ?? 14;
        ...
        return (new MailMessage)
            ...
            ->action('Complete Your Onboarding', url("/hrm/onboarding/{$this->employee->id}"))
            ...
            ->line("Please complete your onboarding within **{$daysRemaining} days**.")
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'action_url' => "/hrm/onboarding/{$this->employee->id}",
        ];
    }
    ...
    {
        $companyName = config('app.name', 'the company');

        return "Welcome to {$companyName}! Tap to start your onboarding.";
    }
}
